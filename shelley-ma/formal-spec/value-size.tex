...
and onwards, the $\ValMonoid$ type replaces $\Coin$ in UTxO map. $\ValMonoid$ terms
...
class $\ValMonoid$, the constraints involving both the serialized size and the upper bound estimate
on the size of a term of any type with $\ValMonoid$ structure must hold.
...
The function $\fun{serSize}$, unlike the functions $\fun{size}$ and $\fun{utxoEntrySize}$
    (explained below), is left abstract. It returns the actual number of bytes a
    term (of a type with $\ValMonoid$ structure)
