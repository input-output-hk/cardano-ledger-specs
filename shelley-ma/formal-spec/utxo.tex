...
\begin{itemize}
  ...
  \item The $\fun{consumed}$ and $\fun{produced}$ calculations are similar to their Shelley
    counterparts, with the following changes: 1) They return terms of $\ValMonoid$, which
...
\begin{itemize}
  ...
  \item The $\ValMonoid$ term in each output is constrained from below by a
    $\ValMonoid$ term that contains some amount of ada and no other tokens.
    The amount of ada that is contained in the constraint $\ValMonoid$ term (and
    hence the $\ValMonoid$ term in the output itself must contain at least as much) depends on the
    ...
    \item The serialized size of the $\ValMonoid$ term in each output is no greater than $\mathsf{MaxValSize}$
    ...
    hold for $\ValMonoid$ instantiated to $\Coin$, as it is a single integer.
\end{itemize}
...
the outputs of a transaction contain a $\ValMonoid$ term, rather than
$\Coin$.
...
$\ValMonoid$. Recall that in Allegra, no assets can ever be minted, so there are no scripts to run that
validate minting.
